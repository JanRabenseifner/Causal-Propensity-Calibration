...
def plot_dist(
    ...
    **kwargs
    ...
    None
    """
    """Create distribution plots with combined headers in bold and large text."""
...
def plot_ate_metrics_old(
    ...
    # Fixed loop - removed enumerate() which caused incorrect data filtering
    for learner_m in learner_m_list:  
        directory = f"{directory_base}{'_'.join(procedures)}/"
        os.makedirs(directory, exist_ok=True)
        ...
        df_filtered = data[
            (data["learner_g"] == learner_g) &
            (data["learner_m"] == learner_m) &  # Now correctly compares strings
            (data["R2_d"] == R2_d) &
            (data["dim_x"] == dim_x) &
            (data["overlap"] == overlap) &
            (data["share_treated"] == share_treated)
        ...
        df_filtered = df_filtered[
            df_filtered["n_obs"].isin(n_obs_list) &
            df_filtered["Method_Clip"].isin(methods_config['methods'])
        ]
        

        # Create figure with guaranteed 2D axes array
        n_rows = len(metrics)
        n_cols = len(procedures)
        if fig_size:
            fig_size=fig_size
        else: fig_size=(5.5*n_cols, 5.5*n_rows)
        fig, axes = plt.subplots(n_rows, n_cols, 
                                figsize=fig_size,
                                sharex=True,
                                squeeze=False)  # Critical fix here
...
def plot_ate_metrics_2(
    ...
    """
    Plot ATE metrics with metrics arranged in rows and procedures in columns
    
    Parameters:
    directory_base (str): Base directory path for saving figures
    data (pd.DataFrame): Input DataFrame containing all analysis data
    procedures (list): List of procedure names to plot (columns)
    metrics (list): List of metric names to plot (rows)
    methods_config (dict): Configuration dictionary containing:
        - 'methods': List of method names to include
        - 'labels': List of method labels for legend
    learner_dict_m (dict): Dictionary of machine learning models for m
    clipping_thresholds (list): List of clipping thresholds to iterate through
    n_obs_list (list): List of sample sizes to include
    dim_x (int): Dimension of covariates
    learner_g (str): Name of learner used for g
    R2_d (float): R-squared value for treatment assignment
    overlap (float): Overlap parameter value
    share_treated (float): Proportion of treated units
    palette_colors (list): Color palette for methods
    add_headers_func (callable): Optional function to add column/row headers
    """
    ...
    for clipping_threshold in clipping_thresholds:
        for learner_m in learner_dict_m:
            ...
            df_filtered = data[np.logical_and.reduce(filter_conds)]
            df_filtered = df_filtered[
                df_filtered["n_obs"].isin(n_obs_list) &
                df_filtered["Method_Clip"].isin(methods_config['methods'])
            ]

            # Split data by procedure
            dfs = [df_filtered[df_filtered["procedure"] == p] for p in procedures]

            # Create figure with metrics in rows
            n_rows = len(metrics)
            n_cols = len(procedures)
            fig, axes = plt.subplots(n_rows, n_cols, 
                                    figsize=(16, 14), 
...
def plot_ate_metrics_3(
    ...
    for clipping_threshold in clipping_thresholds:
        for learner_m in learner_dict_m:
            ...
            # Split data by procedure
            dfs = [df_filtered[df_filtered["procedure"].isin([p])] for p in procedures]
            # Create figure
            fig, axes = plt.subplots(3, 3, figsize=(12, 10), sharex=True)
            axes = axes.flatten()

            # Plot metrics
            for idx, (proc_idx, metric) in enumerate([(i, m) for i in range(3) for m in metrics]):
                ax = axes[idx]
                df_proc = dfs[proc_idx]
            ...
            # Add headers
            if add_headers_func:
                add_headers_func(fig, col_headers=[f'Model = {p}' for p in procedures])
...
def plot_overlap_comparison(
    ...
    if subgroups is not None:
        ...
        plot = create_plot(df, title="Full Sample Overlap")
        filename = (
        f"n_obs_{n_obs:05d}_dim_x_{dim_x:03d}_"
        f"clip_{clipping_threshold:.2f}_"
        f"R2d_{R2_d:.2f}_overlap_{overlap:.2f}_"
...
def plot_overlap_ratio_detailed(
    ...
    share_treated: float,
    panel_labels: bool = True,
    clipping_threshold_val: float = 1e-12,
    n_bins: int = 50,
    **kwargs
):
    """
    ...
    - clipping_threshold_val: Clipping threshold for propensity scores
