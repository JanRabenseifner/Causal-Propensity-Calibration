...
def estimate_irm(data, theta, learner_g, learner_m, n_folds, score, clipping_thresholds, calib_methods, m_0):
    ...
    plr_ses = np.full_like(coefs, fill_value=np.nan)
...
df = pd.DataFrame(columns=[
    "coefs", "ses", "cover", "ci_length", "K", "rmses", "method", "calib_method",
    "ipw_coefs", "plr_coefs", "learner_g", "learner_m",
    "n_obs", "dim_x", "R2_d", "clipping_threshold", "repetition",
...
for i_rep in range(start_rep, end_rep):
    ...
    for i_n_obs, n_obs in enumerate(n_obs_list):
        for i_dim_x, dim_x in enumerate(dim_x_list):
            for i_R2_d, R2_d in enumerate(R2_d_list):
                n_obs = int(n_obs)
                i_rep = int(i_rep)
                dim_x = int(dim_x)
                print(f'Rank {rank} handling repetition: {i_rep + 1} / {n_rep}\tn_obs: {n_obs}\tdim_x: {dim_x}\tR2_d: {R2_d}')
                dgp_dict = {'theta': theta, 'n_obs': n_obs, 'dim_x': dim_x, 'R2_d': R2_d}
...
output_file = f'01_results_irm/ranks_{job_id}/output_rank_{rank:03d}.pkl'
