...
#SBATCH --export=NONE
#SBATCH --output=/beegfs/u/*******/sim_propensity_calibration/01_output_irm/slurm-%j.out 
#SBATCH --error=/beegfs/u/*******/sim_propensity_calibration/01_errors_irm/slurm-%j.err
...
mkdir -p /beegfs/u/*******/sim_propensity_calibration/01_errors_irm
mkdir -p /beegfs/u/*******/sim_propensity_calibration/01_output_irm
mkdir -p /beegfs/u/*******/sim_propensity_calibration/01_results_irm/ranks_$SLURM_JOB_ID
...
output_dir = f'01_results_irm/ranks_{os.getenv("SLURM_JOB_ID")}'
...
combined_df.to_pickle(f'01_results_irm/results_irm_{os.getenv("SLURM_JOB_ID")}.pkl')
EOF
fi
# Print CPUs allowed
echo "CPUs allowed: $(grep Cpus_allowed_list /proc/self/status | awk '{print $2}')" >> /beegfs/u/*******/sim_propensity_calibration/01_output_irm/slurm-$SLURM_JOB_ID.out
